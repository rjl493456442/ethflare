...
func (s *HTTPServer) serveChain(w http.ResponseWriter, r *http.Request) {
...
func (s *HTTPServer) serveChainItem(hash common.Hash) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch shift(&r.URL.Path) {
		case "header":
			// Retrieve the header and attempt to return it
			out, err := s.cluster.ServeRequest(r.Context(), "GetBlockByHash", hash)
			if err == nil {
				block := out.(*types.Block)
				replyAndCache(w, block.Header())
		...
		case "receipts":
			...
			out, err := s.cluster.ServeRequest(r.Context(), "GetReceiptsByHash", hash)
			if err == nil {
